...
export const getAstOnLine = (
	...
	deepestAstNode = children.find((c) =>
		positionSameLineAndNotAfter(c, file, position),
	);
...
export async function nodeFinder<T>(
	...
	const inScope = (ast: ASTBase) => {
		...
		return (
			sortKey !== undefined && (context.getSortKey(ast) ?? -1) <= sortKey
		);
	};
	...
	return issue.issues
		.map((_issue) => {
			switch (_issue) {
				...
				case StandardTypeIssue.PROPERTY_NOT_IN_BINDING:
					return `INTRO name is not defined in the type binding for this node`;
