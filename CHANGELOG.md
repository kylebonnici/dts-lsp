...
- Added diagnostic reporting for multiple includes of the same device tree file.
...
- Fixed loading the correct formatting defaults for `insertFinalNewline` and `trimFinalNewlines`
- Fixed diagnostic errors for the below keyords when `/plugin/` is present:
    - `__overlay__`
    - `__fixups__`
    - `__local_fixups__`
    - `__symbols__`
...
- Downgrade diagnostic severity from error to information when unable to resolve node paths or labels when board file has `/plugin/`
...
- Split `Devicetree: Memory domains view` into two commands
    - `Devicetree: Memory domains view (JSON)`
    - `Devicetree: Memory domains view (Tree)`
...
- Fixed an issue where default settings contributed by third-party extensions were not applied correctly.
...
- Fixed go to definition on `compatible`, It was resolving to the binding file instead of all the place in the dts tree this property is defined.
...
- Fixed VS Code command `devicetree.clipboard.dtMacro` failing to generate result.
...
- Add missing required properties when selection a `compatible`
  from autocomplete list.
    - This can be disabled setting the context setting
      `autoAddMissingPropertiesOnCompletion` to false
...
- Fixed tooltip markdown when tags text was not escaped e.g. `<dma-requests>`
...
- Generate compiled output would not work when triggered on files that are
...
- `device_type` is no longer required on `cpu` nodes.
- When using zephyr binding `dts` folder next to the board file is added to the include list.
- New lines are permitted between labels and node name. This address use case where `wordWrapColumn`
  length is exceed due to the number of labels and node name length.
...
- Fixed formatting issue when labels assigned to a node are pushed on one line even if
  these will exceed wordWrapColumn. Now new lines are permitted.

## [0.7.6] - 2025-12-26

### Fixed

- Stack overflow when an included file is very large.
...
- Improved context initializing time.
...
- Fixed formatting long lines when column length + 1 === wordWrapColumn due to `;`
